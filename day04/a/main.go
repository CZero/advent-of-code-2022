...
func isContained(elves []elf) bool {
	switch {
	case elves[0].s.start >= elves[1].s.start && elves[0].s.stop <= elves[1].s.stop: // Elve 0 contained in Elve 1
		return true
	case elves[1].s.start >= elves[0].s.start && elves[1].s.stop <= elves[0].s.stop: // Elve 1 contained in Elve 2
		return true
	}
	return false
}
