...
// getSharedMultiple get's the shared divider. To find a common divider, you multiply by the numbers, then you have a number which can be divided by all
func getSharedMultiple() {
	for _, monkey := range monkeys {
		sharedMultiple *= monkey.divider
	}
}
...
func doMonkey(n int) {
	...
	for _, item := range monkey.items {
		...
		// We stopped doing relief divisions. Now devide by 10 to keep them managable.
		// item /= 3
		item = item % sharedMultiple
